...
std::vector<float> dsp_window::coswindow(int ntaps, float c0, float c1, float c2)
{
...
std::vector<float> dsp_window::coswindow(int ntaps, float c0, float c1, float c2, float c3)
{
    std::vector<float> taps(ntaps);
    float M = static_cast<float>(ntaps - 1);
...
std::vector<float>
dsp_window::coswindow(int ntaps, float c0, float c1, float c2, float c3, float c4)
{
    std::vector<float> taps(ntaps);
    float M = static_cast<float>(ntaps - 1);
...
std::vector<float> dsp_window::rectangular(int ntaps)
{
    std::vector<float> taps(ntaps);
    for (int n = 0; n < ntaps; n++)
        taps[n] = 1;
...
std::vector<float> dsp_window::hamming(int ntaps)
{
    std::vector<float> taps(ntaps);
    float M = static_cast<float>(ntaps - 1);
...
std::vector<float> dsp_window::hann(int ntaps)
{
    std::vector<float> taps(ntaps);
    float M = static_cast<float>(ntaps - 1);
...
std::vector<float> dsp_window::hanning(int ntaps) { return hann(ntaps); }

std::vector<float> dsp_window::blackman(int ntaps)
{
    return coswindow(ntaps, 0.42, 0.5, 0.08);
}

std::vector<float> dsp_window::blackman2(int ntaps)
{
    return coswindow(ntaps, 0.34401, 0.49755, 0.15844);
}

std::vector<float> dsp_window::blackman3(int ntaps)
{
    return coswindow(ntaps, 0.21747, 0.45325, 0.28256, 0.04672);
}

std::vector<float> dsp_window::blackman4(int ntaps)
{
    return coswindow(ntaps, 0.084037, 0.29145, 0.375696, 0.20762, 0.041194);
}

std::vector<float> dsp_window::blackman_harris(int ntaps, int atten)
{
    switch (atten) {
    case (61):
...
std::vector<float> dsp_window::blackmanharris(int ntaps, int atten)
{
    return blackman_harris(ntaps, atten);
}

std::vector<float> dsp_window::nuttall(int ntaps)
{
    return coswindow(ntaps, 0.3635819, 0.4891775, 0.1365995, 0.0106411);
}

std::vector<float> dsp_window::blackman_nuttall(int ntaps) { return nuttall(ntaps); }
std::vector<float> dsp_window::nuttall_cfd(int ntaps)
{
    return coswindow(ntaps, 0.355768, 0.487396, 0.144232, 0.012604);
}

std::vector<float> dsp_window::flattop(int ntaps)
{
...
std::vector<float> dsp_window::kaiser(int ntaps, double beta)
{
    if (beta < 0)
        return std::vector<float>(ntaps);
...
std::vector<float> dsp_window::bartlett(int ntaps)
{
    std::vector<float> taps(ntaps);
    float M = static_cast<float>(ntaps - 1);
...
std::vector<float> dsp_window::welch(int ntaps)
{
    std::vector<float> taps(ntaps);
...
std::vector<float> dsp_window::parzen(int ntaps)
{
    std::vector<float> taps(ntaps);
    double m1 = midm1(ntaps);
    double m = midn(ntaps);
...
std::vector<float> dsp_window::exponential(int ntaps, double d)
{
    if (d < 0)
        return std::vector<float>(ntaps);
...
std::vector<float> dsp_window::riemann(int ntaps)
{
    double cx;
...
std::vector<float> dsp_window::tukey(int ntaps, float alpha)
{
    if ((alpha < 0) || (alpha > 1))
        return std::vector<float>(ntaps);
...
std::vector<float> dsp_window::gaussian(int ntaps, float sigma)
{
    if (sigma <= 0)
        return std::vector<float>(ntaps);
